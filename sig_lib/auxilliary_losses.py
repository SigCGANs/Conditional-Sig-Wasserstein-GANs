"""
This module contains different auxilliary metrics for tracking the performance of the algorithm.
All of the functions are written in pytorch for efficiency.
The functions are currently undocumented.
"""
...
class Loss(nn.Module):
    def __init__(self, name, reg=0.0, transform=lambda x: x, threshold=10., backward=False, norm_foo=lambda x: x):
...
class ACFLoss(Loss):
    ...
    def compute(self, x_fake):
        ...
        return self.norm_foo(acf_fake - self.acf_real)
...
class HistoLoss(Loss):
    ...
    def compute(self, x_fake):
        ...
        for i in range(x_fake.shape[2]):
            ...
            abs_metric = torch.abs(density - self.densities[i])
...
def init_aux_losses(data, supervision_params):
    aux_losses = list()
    for loss_name, loss_config in supervision_params.items():
        aux_losses.append(supervised_losses[loss_name](x_real=data, **loss_config))
    return aux_losses
